function basewunderground(widget_id, url, skin, parameters)
{
    ...
    title = parameters.title;
		...
		if (parameters.enable_celsius == 1)
		{
			var monitored_entities =
	    [
	        {"entity": "sensor.pws_temp_c", "initial": self.OnStateAvailable, "update": self.OnStateUpdate},
	        ...
	        {"entity": "sensor.pws_temp_high_1d_c", "initial": self.OnStateAvailable, "update": self.OnStateUpdate},
	        {"entity": "sensor.pws_temp_low_1d_c", "initial": self.OnStateAvailable, "update": self.OnStateUpdate},
	        {"entity": "sensor.pws_temp_high_2d_c", "initial": self.OnStateAvailable, "update": self.OnStateUpdate},
	        {"entity": "sensor.pws_temp_low_2d_c", "initial": self.OnStateAvailable, "update": self.OnStateUpdate}
	    ];
		}
		else
		{
    ...
    {
        if (state.entity_id == "sensor.pws_temp_c" || state.entity_id == "sensor.pws_temp_f")
        {
          self.set_field(self, "temperature", state.state);
          self.set_field(self, "unit", state.attributes.unit_of_measurement)
          if (parameters.enable_celsius == 1)
          {
            self.set_field(self, "small_unit", state.attributes.unit_of_measurement.split("C")[0])
          }
          else
          {
            self.set_field(self, "small_unit", state.attributes.unit_of_measurement.split("F")[0])
          }
        }
        if (state.entity_id == "sensor.pws_precip_1d")
        {
          self.set_field(self, "today_prec_icon", "mdi mdi-" + state.attributes.icon.split(":")[1]);
        }
        if (state.entity_id == "sensor.pws_precip_2d")
        {
          self.set_field(self, "tomorrow_prec_icon", "mdi mdi-" + state.attributes.icon.split(":")[1]);
        }
        if (state.entity_id == "sensor.pws_temp_high_1d_f" || state.entity_id == "sensor.pws_temp_high_1d_c")
        {
          self.set_field(self, "today_high", state.state);
        }
        if (state.entity_id == "sensor.pws_temp_low_1d_f" || state.entity_id == "sensor.pws_temp_low_1d_c")
        {
          self.set_field(self, "today_low", state.state);
        }
        if (state.entity_id == "sensor.pws_temp_high_2d_f" || state.entity_id == "sensor.pws_temp_high_2d_c")
        {
          self.set_field(self, "tomorrow_high", state.state);
        }
        if (state.entity_id == "sensor.pws_temp_low_2d_f" || state.entity_id == "sensor.pws_temp_low_2d_c")
        {
    ...
    function set_view(self, state)
    {
        ...
        {
            self.set_field(self, "today_icon", self.weather_icons[state.attributes.entity_picture.split("/")[6].split(".")[0]])
        }
        else if (state.entity_id == "sensor.pws_weather_2d")
        {
            self.set_field(self, "tomorrow_icon", self.weather_icons[state.attributes.entity_picture.split("/")[6].split(".")[0]])
        }
        else
        {
